...
## Frostvein — second biome (spec §4.5: "Frostvein pale teal"). Reuses the BSP
## generator + sprite-per-tile rendering from Catacombs but with a colder
## modulate and the Worm-Mother boss in the exit room.
##
## v0.7.0 deliberately ships Frostvein as a near-clone of Catacombs (same
## scaffolding, different tint + boss) rather than refactoring both into a
## `BiomeBase` first. The shared scaffolding will be extracted Week 8 once
## Lasthold + Cinderfall Spire are also in flight and the diff is concrete.
##
## "Ice-cave variants" (cellular-automata cave generator) — also deferred to
## Week 8/9: the BSP layout suffices to prove the second biome reads
...
const AMBIENT_TINT: Color = Color(0.30, 0.40, 0.55)
const ROOM_LIGHT_COLOR: Color = Color(0.80, 0.95, 1.10)
const ROOM_LIGHT_RANGE_PX: float = 480.0
const ROOM_LIGHT_ENERGY: float = 1.4
const TILE_MODULATE: Color = Color(0.80, 0.95, 1.15)
const ENEMIES_PER_ROOM: int = 1
const MAX_ENEMIES: int = 6
const RNG_SEED_DEFAULT: int = 0xF7053EE
const WORM_HP: int = 400
const WORM_DMG: int = 30
const WORM_ITEM_LEVEL: int = 7
const WORM_DROPS: int = 5
const WORM_DROP_MIN_RARITY: int = 1  # Magic floor
const WORM_SPRITE_VARIANT: int = 3
const ENEMY_SPRITE_VARIANT: int = 4
...
func _render_floor() -> void:
	...
	if tex == null:
		push_error("Frostvein: missing floor tex %s" % FLOOR_TEX_PATH)
		return
...
func _render_walls() -> void:
	...
	if tex == null:
		push_error("Frostvein: missing wall tex %s" % WALL_TEX_PATH)
		return
	var floor_tiles: Dictionary = _dungeon.get("floor_tiles", {})
...
func _spawn_return_portal() -> void:
	...
	portal.portal_color = Color(0.55, 0.80, 0.95, 0.85)
	portal.position = IsoUtils.tile_to_world(entrance + Vector2i(0, -1))
	add_child(portal)


func _spawn_boss() -> void:
	if QuestLog.is_complete("slay_worm_mother"):
		return
	var dungeon_exit: Vector2i = _dungeon.get("exit", Vector2i.ZERO)
	var entrance: Vector2i = _dungeon.get("entrance", Vector2i.ZERO)
	if dungeon_exit == entrance:
		return
	var worm: CharacterBody2D = ENEMY_SCENE.instantiate()
	worm.hp_max = WORM_HP
	worm.attack_damage = WORM_DMG
	worm.item_level = WORM_ITEM_LEVEL
	worm.is_boss = true
	worm.boss_name = "The Worm-Mother"
	worm.quest_on_death = "slay_worm_mother"
	worm.drops_count = WORM_DROPS
	worm.drops_min_rarity = WORM_DROP_MIN_RARITY
	worm.sprite_variant = WORM_SPRITE_VARIANT
	worm.position = IsoUtils.tile_to_world(dungeon_exit)
	add_child(worm)
...
func _log_validation() -> void:
	...
	if unreachable > 0:
		push_warning("Frostvein seed=%d: %d unreachable tiles across %d rooms" % [
			int(_dungeon.get("seed", 0)), unreachable, rooms.size()
		])
	else:
		print("Frostvein seed=%d: %d rooms, %d tiles, fully connected." % [
